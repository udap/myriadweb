...
import { Button, Table, PageHeader, Input, Tag, Divider, message, Modal, Popconfirm, Descriptions, Switch, notification, Pagination, Form, Row, Col, Radio, } from "antd";
import { SearchOutlined, PlusSquareFilled, FolderViewOutlined, ExclamationCircleOutlined, UploadOutlined, } from "@ant-design/icons";

import storageUtils from "../../utils/storageUtils";
import { marketType } from "../../utils/memoryUtils";
import { reqGetMarkets, reqDelCampaign, reqPublishCampaign, reqTransfer, reqDistributions, reqGetNumber, } from "../../api";
...
*/
    ...
    handleFiles = async (files) => {
        ...
        if (this.state.typeStr === "transfer") {
            //transfer 批量分配
            result = await reqTransfer(formData);
            if (result && result.data && result.data.status === "IN_PROGRESS") {
                let str0 = "申请分配" + result.data.requestedAmount + "张票券，后台正在处理中！";
            ...
            result = await reqDistributions(formData);
    ...
    renderContent = () => {
        const {campaigns, size, typeStr, chooseItem, total, currentPage, searchTxt, } = this.state;
